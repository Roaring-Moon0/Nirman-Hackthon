...
const NotificationBell = () => {
    ...
    const markAsRead = async (id) => {
        try {
            ...
            await axios.patch(`${API_BASE_URL}/api/student/notifications/${id}/read`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
        } catch (err) {
    ...
    const markAllRead = async () => {
        try {
            ...
            setUnreadCount(0);
            await axios.patch(`${API_BASE_URL}/api/student/notifications/read-all`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
        } catch (err) {
    ...
    const getIcon = (type, severity) => {
        ...
        if (type === 'risk') return <AlertCircle size={18} className="text-orange-500" />;
        if (type === 'performance') return <AlertTriangle size={18} className="text-yellow-500" />;
                    ...
                    <div className="max-h-[70vh] overflow-y-auto">
                        {notifications.length > 0 ? (
                            notifications.map((n) => (
                                ...
                                >
                                    <div className={`mt-1 flex-shrink-0 ${n.severity === 'critical' ? 'animate-pulse' : ''}`}>
                            ...
                            <div className="p-8 text-center">
                                ...
                                <p className="text-xs text-gray-400 mt-1">No new alerts right now.</p>
                            </div>
                        )}
                    </div>
                </div>
